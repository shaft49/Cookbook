...
inline double triArea(point p, point q, point r) {
...
inline bool normal(const point &a, const point &b) {
    return (EQ(a.x, b.x) ? a.y+EPS < b.y : a.x+EPS < b.x);
}
...
void convexHull(int &np, int &nc) {
    int i, j, pos = 0;
    for(i = 1; i < np; i++)
        if(P[i].y+EPS<P[pos].y || (EQ(P[i].y, P[pos].y) && P[i].x+EPS<P[pos].x))
            pos = i;
    swap(P[0], P[pos]);
    P0 = P[0];
    sort(&P[1], &P[np], comp);
...
void compress(int &nc) {
    ...
    int i, j;
    for(i = j = 1; i < nc; i++) {
        double d = triArea(C[j-1], C[i], C[i+1]);
        if(d != 0.0 || (d == 0.0 && issame(C[j-1], C[i+1]))) C[j++] = C[i];
    }
    nc = j;
}
